...
**Deliberately not fixed, logged rather than silently dropped**: `TrashRepository.bulkDelete()` (11 files) and `contactRepository.js`'s `getKnownAddresses()` are real dead code, and a `FONT_FAMILY_MONO` constant doesn't match its own file's actual usage — none are user-visible, so building UI or wiring for them wasn't attempted this round. Medication Dashboard's Add/Edit forms still don't use `draftStorage.js` (7 sibling forms do), and attachment deletion across 3 files (Attachments screen, Testing's and Clinic Visits' own `AttachmentManager` copies) still has no confirmation — both real, medium-effort gaps flagged for a future round rather than folded in here.
