      ...
      {visibleSuggestions.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 6 }}>
          {visibleSuggestions.map((v) => (
            <div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} key={v} onMouseDown={(ev) => ev.preventDefault()} onClick={() => onChange(v)}
              style={{ padding: "3px 9px", borderRadius: radius.full, fontSize: 11, border: `1px solid ${T.healthcareBlue}`, color: T.healthcareBlue, cursor: "pointer" }}>
              {v}
            </div>
          ))}
...
function DoseByDose({ doses, onChange, T }) {
  ...
  const removeDose = (index) => {
    ...
    newDoses.forEach((d, i) => { d.doseNumber = i + 1; });
    onChange(newDoses);
  };

  const updateDose = (index, key, value) => {
    const newDoses = [...(doses || [])];
    newDoses[index] = { ...newDoses[index], [key]: value };
    onChange(newDoses);
  };
  ...
  if (!doses?.length) {
    return (
      <div style={{ padding: "8px 0" }}>
        <div style={{ fontSize: 12, color: T.textSecondary, marginBottom: 8 }}>No doses logged yet</div>
        <div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} onClick={addDose} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 14px", borderRadius: radius.full, background: T.healthcareBlue, color: "#FFFFFF", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          <Plus size={16} /> Add first dose
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "8px 0" }}>
      <div style={{ fontSize: 12, color: T.textSecondary, marginBottom: 8 }}>Dose series ({doses.length} dose{doses.length > 1 ? "s" : ""})</div>
      {doses.map((dose, index) => (
        <div key={index} style={{ border: `1px solid ${T.border}`, borderRadius: radius.md, background: T.surface, padding: "12px", marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: T.textPrimary }}>Dose {dose.doseNumber}</span>
            <X size={16} color={T.actionRed} style={{ cursor: "pointer" }} onClick={() => removeDose(index)} aria-label="Remove dose" title="Remove dose" role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} />
          </div>
...
const draftKey = `vaccination_${vaccination?.id || "new"}`;
  const [form, setForm] = useState(() => {
    ...
    const base = vaccination ? { ...vaccination } : { ...DEFAULT_VACCINATION, date: new Date().toISOString().slice(0, 10) };
    // Migrate legacy doseNumber/date to doses array if needed
    if (!base.doses?.length && base.doseNumber && base.date) {
      return {
        ...base,
        doses: [{
          doseNumber: base.doseNumber,
          date: base.date,
          time: base.time || null,
          provider: base.provider,
          injectionSite: base.injectionSite,
          notes: base.notes,
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ padding: "0 16px 100px" }}>
        ...
        {v.doses?.length > 0 && (
          <SectionCard title="Dose series" T={T}>
            {v.doses.map((dose, index) => (
              <div key={index} style={{ border: `1px solid ${T.border}`, borderRadius: radius.md, background: T.surface, padding: "12px", marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: T.textPrimary }}>Dose {dose.doseNumber}</span>
                </div>
                <div style={{ fontSize: 12, color: T.textSecondary, marginBottom: 4 }}>
                  {dose.date && <span>{formatDate(dose.date)}</span>}
                  {dose.time && <span style={{ marginLeft: 8 }}>{dose.time}</span>}
                </div>
            ...
            <div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} onClick={() => onEdit(vaccinationId)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 14px", borderRadius: radius.full, background: T.healthcareBlue, color: "#FFFFFF", fontSize: 13, fontWeight: 700, cursor: "pointer", marginTop: 8 }}>
              <Plus size={16} /> Add dose / booster
            </div>
          </SectionCard>
        )}
