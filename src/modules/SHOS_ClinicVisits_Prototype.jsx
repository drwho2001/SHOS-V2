    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span role="button" tabIndex={0} aria-label="Edit clinic visit" style={{ fontSize: 13, fontWeight: 700, color: T.healthcareBlue, cursor: "pointer" }} onClick={() => onEdit(visitId)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }}>Edit</span>
      ...
      {confirmDelete && (
        <ConfirmDeleteCard
          ...
          onConfirm={async () => { await triggerDelete([visit]); refresh(); onBack(); }}
        />
      )}
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {selectMode && (
        <div style={{ background: "#1B1B1F", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            ...
            <span role="button" tabIndex={0} aria-label="Export clinic visit" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} onClick={async () => { if (selectedIds.length === 1) exportRecordAsFile("clinicVisits", await ClinicVisitsRepository.getById(selectedIds[0])); }}
            ...
            <span role="button" tabIndex={0} aria-label="Archive clinic visits" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} onClick={async () => { if (selectedIds.length > 0) { await ClinicVisitsRepository.bulkArchive(selectedIds); syncClinicVisitsToCalendar(await ClinicVisitsRepository.getAll()); refresh(); exitSelectMode(); } }}
            ...
            <span role="button" tabIndex={0} aria-label="Delete clinic visits" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true })); } }} onClick={() => { if (selectedIds.length > 0) setConfirmingBulkDelete(true); }}
      ...
      {confirmingBulkDelete && (
        <ConfirmDeleteCard
          T={T}
          moduleColor={T.healthcareBlue}
          message={`Delete ${selectedIds.length} visit${selectedIds.length > 1 ? "s" : ""} permanently? You'll have a few seconds to undo.`}
          confirmLabel="Delete"
      ...
      <div style={{ position: "fixed", bottom: "calc(90px + env(safe-area-inset-bottom))", left: 0, right: 0, display: "flex", justifyContent: "flex-end", padding: "0 20px", pointerEvents: "none" }}>
        <div onClick={onAdd} role="button" tabIndex={0} aria-label="Add clinic visit" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onAdd(); } }} style={{ width: 56, height: 56, borderRadius: 999, background: T.healthcareBlue, color: "#FFFFFF", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 4px 16px rgba(0,0,0,.2)", pointerEvents: "auto" }}>
          <Plus size={24} />
        </div>
      </div>
